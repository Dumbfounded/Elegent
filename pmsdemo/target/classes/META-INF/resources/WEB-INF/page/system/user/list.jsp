			...
			<c:if test="${requestScope.dwzPageBean != null &&  requestScope.dwzPageBean.recordList != null }">
				<c:forEach var="record" items="${requestScope.dwzPageBean.recordList}" varStatus="status">
					<tr target="id" rel="${record.id}">
						...
						<td>
						    <shiro:hasPermission name="/system/user/toUpdateOrganization">
						       <a title="分配部门" target="dialog" mask="true" href="${pageContext.request.contextPath}/system/user/toUpdateOrganization?id=${record.id}" class="editBtn"><span>分配部门</span></a> 
						    </shiro:hasPermission>
						    <shiro:hasPermission name="/system/user/toUpdateRole">
						       <a title="分配角色" target="dialog" mask="true" href="${pageContext.request.contextPath}/system/user/toUpdateRole?id=${record.id}" class="editBtn"><span>分配角色</span></a> 
						    </shiro:hasPermission>
